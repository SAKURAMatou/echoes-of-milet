<template>
  ...
  >
    ...
    <div class="mt-4 space-y-5">
      <section v-for="city in cities" :key="city.id" class="space-y-3">
        <h2 class="font-serif text-xl leading-tight text-[#26313a]">
          {{ city.name }}
        ...
        <section
          ...
          :key="district.id"
          class="rounded-lg border border-[#dbe7e4] bg-white/54 p-3"
        >
          <h3 class="text-sm font-semibold text-[#315761]">
            {{ district.name }}
            <span class="ml-2 text-xs font-normal text-[#789096]">
...
const copy = computed(() =>
  props.lang === 'jp'
    ? {
        ...
        intro:
          '公開されている milet 聖地巡礼 spot を都市とエリアごとに整理しています。地図上のマーカーと同じ地点、作品、カテゴリ、座標を確認できます。',
        workLabel: '作品',
        coordinateLabel: '座標',
    ...
    : {
        ...
        intro:
          '这里按城市和区划列出已经公开的 milet 圣地巡礼 spot，和地图上的标记保持一致，方便查看作品、分类、封面与坐标信息。',
        workLabel: '作品',
        coordinateLabel: '坐标',
