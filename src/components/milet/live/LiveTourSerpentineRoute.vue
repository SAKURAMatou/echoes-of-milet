<template>
  <article class="grid gap-7">
    ...
    >
      ...
      >
        <div
          ...
          :key="stat.label"
          class="grid min-h-28 place-items-center border-[var(--live-detail-line)] px-4 py-4 text-center lg:border-t first:lg:border-t-0"
        >
          <dt
            class="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--live-detail-muted)]"
          >
            {{ stat.label }}
          </dt>
    ...
    <section class="grid gap-5 lg:grid-cols-[minmax(17rem,23rem)_minmax(0,1fr)]">
      ...
      >
        <p
          class="font-['Montserrat','sans-serif'] text-xs font-semibold uppercase tracking-[0.14em] text-[var(--live-detail-accent)]"
        >
          Selected Stop
        </p>
