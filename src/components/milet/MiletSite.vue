    ...
    <div class="space-y-2">
      <div>
        <a
          ...
          ><img
            src="@/assets/nav-instagram-4.png"
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-5">
    ...
    >
      ...
      <div class="px-4 py-3">
        ...
        <!-- <p class="text-gray-600 text-sm mt-1">记录开发、技术灵感与日常思考</p> -->
      </div>
    </a>
  </div>
</template>
...
onMounted(() => {
  ...
  if (window.twttr && window.twttr.widgets) {
    // 如果 Twitter 脚本已经加载，直接渲染小部件
    window.twttr.widgets.load(twitterContainer.value)
  } else {
    script.src = twScriptLink
    script.async = true
    document.body.appendChild(script)
    // 等待脚本加载完成后渲染 Twitter 小部件
    script.onload = () => {
      if (window.twttr) {
  ...
  insContainer.value.innerHTML = insWidget
  // 动态加载 Instagram 脚本
  const insScript = document.createElement('script')
  insScript.src = 'https://www.instagram.com/embed.js'
